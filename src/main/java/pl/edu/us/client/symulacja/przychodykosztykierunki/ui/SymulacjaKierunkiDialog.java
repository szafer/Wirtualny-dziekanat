...
public class SymulacjaKierunkiDialog extends Dialog {
	...
	public SymulacjaKierunkiDialog(ListStore<ViPrzychod> store) {
		this.store = store;
		czyKoszt = true;
		czyPrzychod = true;
		// Opisy osi chart
		...
		titleMonthYear = new TextSprite("Kierunek studiów");
	...
	protected void onBtnSlupkowyClick() {
	...
	protected void onBtnLiniowyClick() {
		if (chart != null && chart.getParent() != null) {
			chart.removeFromParent();
		}
		ladujAreaChart();

...
public class KartotekaStudentaPanel extends FramedPanel implements /* IsWidget, */Editor<Student> {

	private final KartotekaStudentaModel model;
	private KierunkiStudentaDialog<Kierunek> kierunkiDialog;
	TextField imie, nazwisko, ulica, kodPocztowy, miasto, email, login;
	PasswordField password;
	IntegerField id, nrDomu, nrMieszkania;
	...
	interface ListDriver extends SimpleBeanEditorDriver<Student, KartotekaStudentaPanel> {
	}

	private ListDriver driver = GWT.create(ListDriver.class);

	@Inject
	public KartotekaStudentaPanel(KartotekaStudentaModel model) {
		super();
		this.model = model;
		setBorders(true);
	...
	private void createForm1() {
		...
		id.setWidth(60);
		id.addParseErrorHandler(new ErrorHandler());
		...
		imie.addValueChangeHandler(new FieldHandler<String>());

		nazwisko = new TextField();
		nazwisko.setAllowBlank(false);
		nazwisko.setEmptyText("Prosze wpisać nazwisko...");
		nazwisko.addValueChangeHandler(new FieldHandler<String>());
		...
		plec.addValueChangeHandler(new FieldHandler<Plec>());
		...
		ulica.addValueChangeHandler(new FieldHandler<String>());

		nrDomu = new IntegerField();
		nrDomu.setAllowBlank(false);
		nrDomu.setWidth(50);
		nrDomu.addParseErrorHandler(new ErrorHandler());

		nrMieszkania = new IntegerField();
		nrMieszkania.setAllowBlank(false);
		nrMieszkania.setWidth(50);
		nrMieszkania.addParseErrorHandler(new ErrorHandler());
		...
		miasto.addValueChangeHandler(new FieldHandler<String>());
		...
		vlc.add(new FieldLabel(id, "Nr albumu"));
